...
\Ensure{A list $D$ of distances of all vertices from $s$. A tree $T$ rooted at $s$.}
	\State $Q \leftarrow [s]$
	\Comment{queue of nodes to visit}
	\State $D \leftarrow [\infty, \infty, \dots, \infty]$
	\Comment{$n$ copies of $\infty$}	
...
\Ensure{A list $D$ of distances of all vertices from $s$. A tree $T$
  rooted at $s$.}
	\State $S \leftarrow [s]$
	\Comment{stack of nodes to visit}
	\State $D \leftarrow [\infty, \infty, \dots, \infty]$
	\Comment{$n$ copies of $\infty$}	
	\State $D[s] \leftarrow 0$	
	\State $T \leftarrow [\,]$
